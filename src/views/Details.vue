<template>
  <div class="details">
    ...
    <!-- 1 房屋轮播图 -->
    <div class="banner">
      <mt-swipe :showIndicators="true" :auto="4000" :speed="600">
        <mt-swipe-item>
    ...
    <div class="main">
      ...
      <div class="pattern">
        ...
        <div class="left">
          ...
          <span>
            {{ info.r_room }}室 {{ info.r_hall }}厅 {{ info.r_toilet }}卫
          </span>
        </div>
      ...
      <div class="time">
        <van-cell-group>
          ...
          <van-cell
            :value="`${this.checkIn} 14:00  至  ${this.checkOut} 12:00  共${this.liveCount}晚`"
          />
        </van-cell-group>
      </div>
      <!-- 6 配套设施 -->
      ...
      <div class="homeowner">
        ...
        <div class="old_7">
          <img src="" alt="房东头像" />
          <span class="homeower_name">房东如意会所</span><br />
          <hr />
      ...
      <div class="comment">
        ...
        <div class="old_8">
          ...
          <span class="comment_content"
            >客厅、休闲角、起居室、厨房、上下层2个卫生间的每一寸设计，都为外出旅行的你提供尽可能所需。
            【成都土著少女房东】将你出行的每个小细节都妥帖响应，呈现在【墨宿
            more homie】的角角落落。
            4.9米挑高的矩形落地窗漫射着冬季的旭日暖阳，毫无遮拦地洒进窗内。</span
          >
        </div>
      </div>
      <!-- 9 预定须知 -->
      <div class="notice">
        ...
        <div class="old_9">
          ...
          <span class="notice_content"
            >入住日14:00后入住 退订日12:00前离开(以当地城市时区为准)</span
          >
          <br />
          <!-- 规则2 -->
    ...
    <div class="footer">
      <router-link :to="`/paypage/${info.rid}`">
        <van-submit-bar
          :price="
            liveCount ? info.r_price * liveCount * 100 : info.r_price * 100
          "
          button-text="提交订单"
          @submit="onSubmit"
        />
...
.details .pattern .left,
.details .pattern .right {
  display: inline-block;
}
.details .pattern img {
  margin: 0 5px 10px 40px;
}
.details .pattern .left span,
.details .pattern .right span {
  display: block;
  font-size: 14px;
  text-align: left;
}
...
export default {
  ...
  methods: {
    ...
    onConfirm(date) {
      ...
      this.liveCount =
        (Date.parse(this.formatDate(end)) -
          Date.parse(this.formatDate(start))) /
        1000 /
        60 /
        60 /
        24;
